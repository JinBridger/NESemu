...
bool CPU::ExecuteType1(Byte opcode) {
  if ((opcode & InstructionModeMask) == 0x1) {
    ...
    switch (static_cast<AddressingMode1>((opcode & AddrModeMask) >>
                                         AddrModeShift)) {
...
bool CPU::ExecuteType0(Byte opcode) {
  if ((opcode & InstructionModeMask) == 0x0) {
    ...
    // {
    //     LOG(Info) << "Fetch Instruction :" << std::hex <<
    //     static_cast<int>(DEBUG_OPCODE) << std::endl;
    // }
    switch (static_cast<AddressingMode2>((opcode & AddrModeMask) >>
                                         AddrModeShift)) {
      case Immediate_:
        location = r_PC++;
        break;
